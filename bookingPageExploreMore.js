...
const activityContainer = document.getElementById('carouselContainer');

activities.forEach(activity => {
    const activityElement = document.createElement('a');
    activityElement.href = `BookingPage.html?id=${activity.id}`;
    activityElement.className = 'carousel-item activity-card block bg-white p-4 rounded-lg shadow group transition duration-300 transform hover:bg-gray-200';
